...
async function main() {
  ...
  // 1. Canonical Creative Brief (no offer: offer = "")
  ...
  console.log('Offer:', canonicalBrief.offer || '(None)');
  ...
  const outPath = path.join(outDir, 'bts-return-event.png');
  ...
  console.log('Blind Critic Evaluation:', {
    ...
    observedEvent: result.critic.observedEvent,
    observedOffer: result.critic.observedOffer,
    observedHero: result.critic.observedHero,
    templateLook: result.critic.templateLook,
    humanCraft: result.critic.humanCraft,
    reasonsToReject: result.critic.problems,
