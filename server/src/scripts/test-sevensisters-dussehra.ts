...
async function main() {
  console.log('=== Starting Live SevenSisters Dussehra End-to-End Generation ===');

  // Find brand for SevenSisters if in DB
  let brand: any = null;
  try {
    ...
    console.log('DB query skipped / unavailable, continuing with SevenSisters identity fallback.');
  }

  const userId = brand?.userId || brand?.created_by || brand?.ownerId || 'ddbead95-388b-440c-b4d4-559dd712f9c0';
  const brandId = brand?.id;
  console.log('Found Brand row:', brand);
  console.log(`Using User ID: ${userId}`);
  console.log(`Using Brand: ${brand?.name || 'SevenSisters'} (ID: ${brandId || 'None'})`);
  ...
  const prompt = 'A grand Dussehra festive campaign for SevenSisters — celebrating the triumph of light, rich authentic culinary craft, and welcoming our community to feast.';

  // 1. Discover Concepts (/concepts)
  ...
  const discoveryResult = await creativeGenerationService.discoverConcepts(userId, {
    ...
    creativeDna: {
      ...
      mood: 'editorial, warm, elevated, celebratory',
    },
    brandVoice: {
      name: 'SevenSisters',
      description: 'Contemporary Asian dining celebrating culinary heritage with modern craft.',
      tone: 'warm, elevated, festive, editorial',
      wordsToUse: ['craft', 'heritage', 'light', 'feast', 'table', 'togetherness'],
    },
  });
  ...
  // 2. Generate Final Creative (/generate)
  ...
  console.log(`\n======================================================`);
  console.log(`SUCCESS! Generated SevenSisters Dussehra Campaign!`);
  ...
  console.log(`Brand Message: "${asset.creativeBrief.marketingCreative?.brandMessage}"`);
