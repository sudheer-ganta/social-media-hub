...
export interface StyleDNA {
  ...
  renderer: {
    ...
    compositionArchetypes: CompositionArchetype[];
  };
...
const render = (
  typographyFamily: RecipeTypographyFamily[],
  compositionArchetypes: CompositionArchetype[],
  options: Partial<StyleDNA['renderer']> = {},
): StyleDNA['renderer'] => ({
  typographyFamily,
  compositionArchetypes,
  logoTreatment: ['corner', 'footer'],
  spacing: ['generous'],
  footer: ['none', 'hairline'],
  border: ['none'],
  shapeLanguage: ['none', 'editorial-rules'],
  imageTreatment: ['full-bleed', 'inset'],
  ...options,
});
...
export const STYLE_PALETTES: Record<StyleDnaId, string[][]> = {
  minimalist: [
    ['#f8f7f4', '#18181b', '#b45309'],
    ['#f4f4f5', '#09090b', '#71717a'],
  ],
  editorial: [
    ['#faf6f0', '#1c1917', '#881337'],
    ['#f5f5f4', '#0f172a', '#ca8a04'],
  ],
  y2k: [
    ['#e0f2fe', '#030712', '#ec4899', '#38bdf8'],
    ['#0f172a', '#f8fafc', '#f43f5e', '#a855f7'],
  ],
  'desi-maximalism': [
    ['#fef3c7', '#1c1917', '#be123c', '#f59e0b'],
    ['#fffbeb', '#0f172a', '#4338ca', '#ea580c'],
  ],
  'neo-brutalism': [
    ['#ffffff', '#000000', '#facc15'],
    ['#000000', '#ffffff', '#ef4444'],
  ],
  'minimal-doodles': [
    ['#fbfaf7', '#1e293b', '#0d9488'],
    ['#ffffff', '#0f172a', '#f97316'],
  ],
  'imperfect-handmade': [
    ['#f5eedc', '#27272a', '#c2410c'],
    ['#fefce8', '#1c1917', '#854d0e'],
  ],
  'bold-typography': [
    ['#ffffff', '#09090b', '#dc2626'],
    ['#f8fafc', '#0f172a', '#ea580c'],
  ],
  'cinematic-drama': [
    ['#0f172a', '#f8fafc', '#f59e0b'],
    ['#18181b', '#f4f4f5', '#0d9488'],
  ],
  'creator-ugc': [
    ['#fafaf9', '#18181b', '#6366f1'],
    ['#f5f5f4', '#0f172a', '#f43f5e'],
  ],
  'vibrant-color-blocking': [
    ['#fef08a', '#0f172a', '#2563eb', '#f43f5e'],
    ['#ede9fe', '#020617', '#16a34a', '#ec4899'],
  ],
  luxury: [
    ['#fcfbf9', '#171717', '#d97706'],
    ['#1c1917', '#fafaf9', '#ca8a04'],
  ],
  'retro-nostalgic': [
    ['#fdf6e2', '#292524', '#ea580c', '#d97706'],
    ['#fef3c7', '#1c1917', '#15803d', '#c2410c'],
  ],
  collage: [
    ['#f4ede4', '#18181b', '#e11d48', '#0284c7'],
    ['#fefae0', '#283618', '#bc6c25', '#dda15e'],
  ],
  '3d-futuristic': [
    ['#0b0f19', '#f1f5f9', '#8b5cf6', '#06b6d4'],
    ['#ffffff', '#030712', '#3b82f6', '#10b981'],
  ],
};
...
export interface ChooseCompositionArchetypeInput {
  style: StyleDNA;
  variant: number;
...
/**
 * Deterministically selects the composition archetype.
 * Single source of truth: Style DNA owns the allowed archetype pool.
 * Concept characteristics and image capabilities guide filtering, but the
 * selected archetype ALWAYS remains within the Style DNA's valid pool.
 */
export function chooseCompositionArchetype(input: ChooseCompositionArchetypeInput): CompositionArchetype {
  const pool = input.style.renderer.compositionArchetypes;
  if (!pool || pool.length === 0) return 'FULL_BLEED_TYPE';

  // 1. Image capability filtering:
  // PRODUCT_CUTOUT must not be selected when the renderer cannot actually fulfill the cutout requirement.
  let candidates = pool.filter((arch) => {
    if (arch === 'PRODUCT_CUTOUT' && input.capabilities && input.capabilities.isCutout === false) {
      return false;
    }
    return true;
  });
  ...
  // 2. Concept compatibility filtering / scoring:
  if (input.concept) {
    const family = input.concept.artDirectionFamily;
    const copyTreatment = input.concept.copyTreatment;
    const hasLongCopy = Boolean(input.concept.headline && (input.concept.supportingLine || input.concept.marketingCreative?.brandMessage));

    let preferred: CompositionArchetype[] = [];
    if (family === 'TYPOGRAPHY_LED') {
      preferred = ['TYPOGRAPHIC_POSTER', 'FULL_BLEED_TYPE'];
    } else if (family === 'COLLAGE' || family === 'HANDCRAFTED') {
      preferred = ['COLLAGE_LAYERED', 'FRAME_WITH_OVERLAP'];
    } else if (family === 'MINIMAL_ART') {
      preferred = ['NEGATIVE_SPACE', 'FULL_BLEED_TYPE', 'SPLIT_COMPOSITION'];
    } else if (family === 'EDITORIAL_PHOTOGRAPHY' || family === 'CULTURAL_EDITORIAL') {
      preferred = ['EDITORIAL_OVERLAP', 'ASYMMETRIC_GRID', 'FULL_BLEED_TYPE'];
    } else if (family === 'PRODUCT_STUDIO') {
      preferred = ['PRODUCT_CUTOUT', 'ASYMMETRIC_GRID', 'NEGATIVE_SPACE'];
    } else if (copyTreatment === 'none') {
      preferred = ['FULL_BLEED_TYPE', 'NEGATIVE_SPACE'];
    } else if (hasLongCopy) {
      preferred = ['ASYMMETRIC_GRID', 'EDITORIAL_OVERLAP', 'SPLIT_COMPOSITION'];
    }

    const matched = candidates.filter((c) => preferred.includes(c));
    if (matched.length > 0) {
      candidates = matched;
    }
  }
  ...
  return candidates[Math.abs(input.variant) % candidates.length];
}

export function styleDnaToRecipe(
  style: StyleDNA,
  variant: number,
  options?: { concept?: CreativeDirection; capabilities?: ImageCapabilities },
): ReferenceDesignRecipe {
  const archetype = chooseCompositionArchetype({
    style,
    variant,
  ...
  const palettes = STYLE_PALETTES[style.id] ?? STYLE_PALETTES.minimalist;
  const colorPalette = choose(palettes, variant, 0);
  ...
  return {
    ...
    compositionBehaviour: choose(style.composition, variant, 3),
    textHierarchy: `${style.typography.hierarchy} hierarchy, ${style.typography.textDensity} text density`,
    typographyFamily: choose(style.renderer.typographyFamily, variant, 4),
    colorPalette,
    layoutBehaviour: choose(style.layout.layoutBehaviour, variant, 5),
    logoTreatment: choose(style.renderer.logoTreatment, variant, 6),
    spacingBehaviour: choose(style.renderer.spacing, variant, 7),
    texture: choose(style.texture, variant, 8),
    graphicElements: style.graphicElements.slice(0, 4),
    footerStyle: choose(style.renderer.footer, variant, 9),
    borderStyle: choose(style.renderer.border, variant, 10),
    shapeLanguage: choose(style.renderer.shapeLanguage, variant, 11),
    visualDensity: style.layout.density,
    imperfectionLevel: style.imperfection.level,
    imageTreatment: choose(style.renderer.imageTreatment, variant, 12),
...
export function validateStyleDNA(value: unknown): string[] {
  ...
  if (!style.renderer?.compositionArchetypes?.length) errors.push('composition archetypes are required in renderer');
