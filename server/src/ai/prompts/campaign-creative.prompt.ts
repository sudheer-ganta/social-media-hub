...
export function buildCampaignQcPrompt(copy: CampaignCopyLine[], requiredClaims: string[]): string {
  return [
    ...
    '- Text with poor color contrast against its backdrop, making it difficult or painful to read (e.g. orange text on a peach/beige background).',
    '- Low-quality graphic devices, such as cartoon ribbon banners, flat clip-art stickers, or hand-drawn stamps that clash with the premium photography style.',
    ...
    '',
    'Rate each defect:',
    '- "critical" — the creative says the wrong thing or is unpublishable: a listed line missing, misspelled or altered; a required claim illegible; an invented logo; the headline unreadable; text colliding with text; placeholder/template artifacts; text with insufficient color contrast; cheap clip-art graphic elements.',
...
export function buildCampaignCreativePrompt(context: CampaignCreativeContext): string {
  ...
  const lines = [
    ...
    '- AVOID STATIC SIDE-BY-SIDE SPLITS: Do not split the canvas into a boring template layout where the photo is entirely on one side and a plain block of text is on the other. Instead, weave the design elements organically: overlay text across the visual using translucent panels, offset titles, wrap copy around the subject, or bleed the photo under a structured typographic grid. Make the graphic design and the imagery feel unified as a single custom composition.',
    '',
    'NEVER PRODUCE:',
    ...
    '- Text with poor color contrast against the visual behind it. Saturated/bright text colors must never sit directly on similar-valued backgrounds. Use dark ink text (e.g., deep charcoal) on light backdrops, and white/cream text on dark backdrops, or add a solid backing plate/scrim to ensure high readability.',
    '- Low-quality clip-art vector elements, such as cartoon banner ribbons, fake stamps, or clip-art stickers. All graphic devices must look premium, modern, cleanly integrated, and designed by a human art director.',
